...
inline constexpr unsigned makerop4(unsigned fore, unsigned back) noexcept { return ((back << 8) & 0xFF000000) | fore; }
...
struct drawpatrect {
  point position, size;
  unsigned short style, pattern;
};
static_assert(sizeof(drawpatrect) == sizeof(DRAWPATRECT));
...
struct psinjectdata {
  unsigned databytes;
  unsigned short injectionpoint, pagenumber;
};
static_assert(sizeof(psinjectdata) == sizeof(PSINJECTDATA));
...
struct psfeature_output {
  int pageindependent, setpagedevice;
};
static_assert(sizeof(psfeature_output) == sizeof(PSFEATURE_OUTPUT));

// Information about custom paper size
struct psfeature_custpaper {
  int orientation, width, height, widthoffset, heightoffset;
};
static_assert(sizeof(psfeature_custpaper) == sizeof(PSFEATURE_CUSTPAPER));
...
static_assert(sizeof(xform) == sizeof(XFORM));

// Bitmap Header Definition
struct bitmap {
  int type, width, height, widthbytes;
  unsigned short planes, bitspixel;
  void* bits;
};
static_assert(sizeof(bitmap) == sizeof(BITMAP));

struct rgbtriple {
  unsigned char blue, green, red;
};
static_assert(sizeof(rgbtriple) == sizeof(RGBTRIPLE));

struct rgbquad {
  unsigned char blue, green, red, reserved;
};
static_assert(sizeof(rgbquad) == sizeof(RGBQUAD));
...
inline constexpr unsigned cmyk(unsigned c, unsigned m, unsigned y, unsigned k) noexcept { return k | (y << 8) | (m << 16) | (c << 24); }

struct ciexyz {
  int x, y, z;
};
static_assert(sizeof(ciexyz) == sizeof(CIEXYZ));

struct ciexyztriple {
  ciexyz ciexyzRed, ciexyzGreen, ciexyzBlue;
};
static_assert(sizeof(ciexyztriple) == sizeof(CIEXYZTRIPLE));
...
struct logcolorspace {
  unsigned signature, version, size;
  int cstype, intent;
...
typedef enum { DISPLAYCONFIG_SCANLINE_ORDERING_UNSPECIFIED = 0, DISPLAYCONFIG_SCANLINE_ORDERING_PROGRESSIVE = 1, DISPLAYCONFIG_SCANLINE_ORDERING_INTERLACED = 2, DISPLAYCONFIG_SCANLINE_ORDERING_INTERLACED_UPPERFIELDFIRST = DISPLAYCONFIG_SCANLINE_ORDERING_INTERLACED, DISPLAYCONFIG_SCANLINE_ORDERING_INTERLACED_LOWERFIELDFIRST = 3, DISPLAYCONFIG_SCANLINE_ORDERING_FORCE_UINT32 = 0xFFFFFFFF } DISPLAYCONFIG_SCANLINE_ORDERING;
...
typedef enum { DISPLAYCONFIG_SCALING_IDENTITY = 1, DISPLAYCONFIG_SCALING_CENTERED = 2, DISPLAYCONFIG_SCALING_STRETCHED = 3, DISPLAYCONFIG_SCALING_ASPECTRATIOCENTEREDMAX = 4, DISPLAYCONFIG_SCALING_CUSTOM = 5, DISPLAYCONFIG_SCALING_PREFERRED = 128, DISPLAYCONFIG_SCALING_FORCE_UINT32 = 0xFFFFFFFF } DISPLAYCONFIG_SCALING;

typedef enum { DISPLAYCONFIG_ROTATION_IDENTITY = 1, DISPLAYCONFIG_ROTATION_ROTATE90 = 2, DISPLAYCONFIG_ROTATION_ROTATE180 = 3, DISPLAYCONFIG_ROTATION_ROTATE270 = 4, DISPLAYCONFIG_ROTATION_FORCE_UINT32 = 0xFFFFFFFF } DISPLAYCONFIG_ROTATION;

typedef enum { DISPLAYCONFIG_MODE_INFO_TYPE_SOURCE = 1, DISPLAYCONFIG_MODE_INFO_TYPE_TARGET = 2, DISPLAYCONFIG_MODE_INFO_TYPE_DESKTOP_IMAGE = 3, DISPLAYCONFIG_MODE_INFO_TYPE_FORCE_UINT32 = 0xFFFFFFFF } DISPLAYCONFIG_MODE_INFO_TYPE;

typedef enum { DISPLAYCONFIG_PIXELFORMAT_8BPP = 1, DISPLAYCONFIG_PIXELFORMAT_16BPP = 2, DISPLAYCONFIG_PIXELFORMAT_24BPP = 3, DISPLAYCONFIG_PIXELFORMAT_32BPP = 4, DISPLAYCONFIG_PIXELFORMAT_NONGDI = 5, DISPLAYCONFIG_PIXELFORMAT_FORCE_UINT32 = 0xffffffff } DISPLAYCONFIG_PIXELFORMAT;
...
typedef enum DISPLAYCONFIG_TOPOLOGY_ID { DISPLAYCONFIG_TOPOLOGY_INTERNAL = 0x00000001, DISPLAYCONFIG_TOPOLOGY_CLONE = 0x00000002, DISPLAYCONFIG_TOPOLOGY_EXTEND = 0x00000004, DISPLAYCONFIG_TOPOLOGY_EXTERNAL = 0x00000008, DISPLAYCONFIG_TOPOLOGY_FORCE_UINT32 = 0xFFFFFFFF } DISPLAYCONFIG_TOPOLOGY_ID;
...
WINSPOOLAPI
int WINAPI DeviceCapabilitiesA(_In_ LPCSTR pDevice, _In_opt_ LPCSTR pPort, _In_ WORD fwCapability, _Out_writes_opt_(_Inexpressible_(1)) LPSTR pOutput, _In_opt_ CONST DEVMODEA* pDevMode);
WINSPOOLAPI
int WINAPI DeviceCapabilitiesW(_In_ LPCWSTR pDevice, _In_opt_ LPCWSTR pPort, _In_ WORD fwCapability, _Out_writes_opt_(_Inexpressible_(1)) LPWSTR pOutput, _In_opt_ CONST DEVMODEW* pDevMode);
#ifdef UNICODE
