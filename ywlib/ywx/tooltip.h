...
public:
  ...
  std::expected<void, error_trace> initialize() {
    ...
    if (auto res = text_layout::create(L"")) _layout = std::move(*res);
    else return unexpected_error(res.error());
    _layout.text_alignment(DWRITE_TEXT_ALIGNMENT_CENTER);
    _layout.paragraph_alignment(DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
    return {};
  }
  ...
  std::expected<void, error_trace> show(int2 Pos, uint2 Size, std::wstring_view Text) {
    _visible = true;
    if (auto res = initialize(); !res) return unexpected_error(res.error());
    if (auto res = text_layout::create(Text, _layout, none())) _layout = std::move(*res);
    else return unexpected_error(res.error());
