...
public:
  ...
  template<arithmetic T> T gen() noexcept {
    if constexpr (uint_type<T>) return uint<T>();
    else if constexpr (int_type<T>) return static_cast<T>(uint_cast(T()));
    else return real<T>();
  }

  template<arithmetic T> T gen(T Max) noexcept {
    if constexpr (float_type<T>) return gen<T>(T(0), Max);
    else return gen<T>(T(0), Max);
  }

  template<arithmetic T> T gen(T Min, T Max) noexcept {
...
template<arithmetic T> T gen() noexcept { return default_generator.gen<T>(); }
template<arithmetic T> T gen(T Max) noexcept { return default_generator.gen<T>(Max); }
template<arithmetic T> T gen(T Min, T Max) noexcept { return default_generator.gen<T>(Min, Max); }
