...
//     /// 所属するウィンドウID
//     slotid window_id{};
//     /// サイズに加えて要求する余白 (left, top, right, bottom)
...
//     /// コントロールの最小サイズ (セット時にゼロ以上を約束すること)
//     float2 min_size = float2::fill(arbitrary_value * 2);
//     /// provided_areaに余剰がある場合の配置
//     ui::alignment alignment = ui::alignment::center;
//     /// サイズが固定されているかどうか
//     vector<bool, 2> constrained{false, false};
//     /// 可視性、操作可能性、削除予定かどうか
...
//     /// 所属ウィンドウにレイアウト再構築を要求する
//     void make_messy() const noexcept;

//     /// alignmentを元に描画位置を更新する
//     void update_align() {
//       static const float c[] = {0.5f, 0.0f, 1.0f}; // center, left, right
//       const float2 cc{c[unsigned(alignment) % 3], c[unsigned(alignment) >> 2 % 3]};
//       pos = provided_pos + (provided_size - (size + margin.xy() + margin.zw())) * cc;
//     }
//     /// デストラクタ。直属レイアウトとシステムに対してそれぞれ登録削除を行う
//     virtual ~slot() noexcept {
//       if (const auto lsp = system::slot_address<slot>(layout_id); lsp && !lsp->dying) lsp->detach(id);
//       system::uis.erase(id);
//     }
//     /// ヒットテスト結果。Pointがジオメトリ内であれば、このコントロールのIDを返す
//     virtual slotid hittest(float2 Point) const {
//       if (!visible || !geometry) return {};
//       BOOL hit = false;
//       const auto hr = geometry->FillContainsPoint(D2D1_POINT_2F{Point.x, Point.y}, D2D1::Matrix3x2F::Identity(), &hit);
//       return SUCCEEDED(hr) && hit ? id : slotid{};
//     }
//     /// コントロールの必要最小サイズを返す
//     virtual float2 calculate_min_size() const { return vapply_r<float2>(yw::max, min_size, size * constrained); }
...
//     }
//     /// レイアウトを更新する
//     virtual std::expected<void, error_trace> update_layout(float2 Pos, float2 Size) {
//       provided_pos = Pos, provided_size = Size;
//       size = (float2(1.0f, 1.0f) - constrained) * (Size - (margin.xy() + margin.zw())) + constrained * size;
//       update_align();
//       return update_geometry();
//     }
//     /// フォーカスを持ったコントロールを探し、さらに次以降のフォーカスを持てるコントロールを探す
//     virtual slotid next_tab_stop(slotid Current, bool Forward, bool& Found) {
//       if (Current == id) Found = true;
//       else if (Found && visible && enabled && focusable()) return id;
//       return {};
//     }

//     virtual std::expected<void, error_trace> draw() const {}
//     virtual std::expected<void, error_trace> draw_focus_ring() const {}
//     virtual bool focusable() const noexcept { return false; }
//     virtual float2 ime_position() const { return {}; };
//     virtual std::expected<void, error_trace> ime_insert_text(std::wstring_view) {}
...
//   float2 pos() const { return unsafe_get(&slot::pos); }
//   float2 size() const { return unsafe_get(&slot::size); }

//   float2 radius() const { return unsafe_get(&slot::radius); }
//   auto& radius(float2 Radius) { return safe_set_geometry(&slot::radius, Radius), *this; }

//   float2 min_size() const { return unsafe_get(&slot::min_size); }
//   auto& min_size(float2 MinSize) { return safe_set_layout(&slot::min_size, MinSize), *this; }

//   ui::alignment alignment() const { return unsafe_get(&slot::alignment); }
//   auto& alignment(ui::alignment Alignment) { return safe_set(&slot::alignment, Alignment), *this; }
// };
// } // namespace yw::ui
