...
inline const wndproc defwindowproc = ::DefWindowProcW;

/// \brief displays a message box with an OK button
inline bool ok(const format_string<cat1> Text, const format_string<cat1> Caption = "OK?") noexcept { //
  return ::MessageBoxA(nullptr, Text.get().data(), Caption.get().data(), MB_ICONEXCLAMATION | MB_OK) == IDOK;
}

/// \brief displays a message box with an OK button
inline bool ok(const format_string<cat2> Text, const format_string<cat2> Caption = L"OK?") noexcept { //
  return ::MessageBoxW(nullptr, Text.get().data(), Caption.get().data(), MB_ICONEXCLAMATION | MB_OK) == IDOK;
}

/// \brief displays a message box with an OK button
inline bool ok(stringable auto&& Text, stringable auto&& Caption) noexcept                                                      //
  requires (!(convertible_to<decltype(Text), format_string<cat1>> && convertible_to<decltype(Caption), format_string<cat1>>) && //
            !(convertible_to<decltype(Text), format_string<cat2>> && convertible_to<decltype(Caption), format_string<cat2>>)) {
  using t1 = remove_cvref<iter_value<decltype(Text)>>;
  using t2 = remove_cvref<iter_value<decltype(Caption)>>;
  if constexpr (same_as<cat1, t1, t2>) {
    string<cat1> s1(string_view<cat1>(Text)), s2(string_view<cat1>(Caption));
    return ::MessageBoxA(nullptr, s1.c_str(), s2.c_str(), MB_ICONEXCLAMATION | MB_OK) == IDOK;
  } else {
    string<cat2> s1(cvt<cat2>(Text)), s2(cvt<cat2>(Caption));
    return ::MessageBoxW(nullptr, s1.c_str(), s2.c_str(), MB_ICONEXCLAMATION | MB_OK) == IDOK;
  }
}

/// \brief displays a message box with an OK button
inline bool ok(stringable auto&& Text) noexcept //
  requires (!(convertible_to<decltype(Text), format_string<cat1>>) && !(convertible_to<decltype(Text), format_string<cat2>>)) {
  using t = remove_cvref<iter_value<decltype(Text)>>;
  if constexpr (same_as<cat1, t>) {
    string<cat1> s(string_view<cat1>(Text));
    return ::MessageBoxA(nullptr, s.c_str(), "OK?", MB_ICONEXCLAMATION | MB_OK) == IDOK;
  } else {
    string<cat2> s(cvt<cat2>(Text));
    return ::MessageBoxW(nullptr, s.c_str(), L"OK?", MB_ICONEXCLAMATION | MB_OK) == IDOK;
  }
}

/// \brief displays a message box with Yes and No buttons
[[nodiscard]] inline bool yes(const format_string<cat1> Text, const format_string<cat1> Caption = "Yes?") noexcept { //
  return ::MessageBoxA(nullptr, Text.get().data(), Caption.get().data(), MB_ICONQUESTION | MB_YESNO) == IDYES;
}

/// \brief displays a message box with Yes and No buttons
[[nodiscard]] inline bool yes(const format_string<cat2> Text, const format_string<cat2> Caption = L"Yes?") noexcept { //
  return ::MessageBoxW(nullptr, Text.get().data(), Caption.get().data(), MB_ICONQUESTION | MB_YESNO) == IDYES;
}

/// \brief displays a message box with Yes and No buttons
[[nodiscard]] inline bool yes(stringable auto&& Text, stringable auto&& Caption) noexcept                                       //
  requires (!(convertible_to<decltype(Text), format_string<cat1>> && convertible_to<decltype(Caption), format_string<cat1>>) && //
            !(convertible_to<decltype(Text), format_string<cat2>> && convertible_to<decltype(Caption), format_string<cat2>>)) {
  using t1 = remove_cvref<iter_value<decltype(Text)>>;
  using t2 = remove_cvref<iter_value<decltype(Caption)>>;
  if constexpr (same_as<cat1, t1, t2>) {
    string<cat1> s1(string_view<cat1>(Text)), s2(string_view<cat1>(Caption));
    return ::MessageBoxA(nullptr, s1.c_str(), s2.c_str(), MB_ICONQUESTION | MB_YESNO) == IDYES;
  } else {
    string<cat2> s1(cvt<cat2>(Text)), s2(cvt<cat2>(Caption));
    return ::MessageBoxW(nullptr, s1.c_str(), s2.c_str(), MB_ICONQUESTION | MB_YESNO) == IDYES;
  }
}

/// \brief displays a message box with Yes and No buttons
[[nodiscard]] inline bool yes(stringable auto&& Text) noexcept //
  requires (!(convertible_to<decltype(Text), format_string<cat1>>) && !(convertible_to<decltype(Text), format_string<cat2>>)) {
  using t = remove_cvref<iter_value<decltype(Text)>>;
  if constexpr (same_as<cat1, t>) {
    string<cat1> s(string_view<cat1>(Text));
    return ::MessageBoxA(nullptr, s.c_str(), "Yes?", MB_ICONQUESTION | MB_YESNO) == IDYES;
  } else {
    string<cat2> s(cvt<cat2>(Text));
    return ::MessageBoxW(nullptr, s.c_str(), L"Yes?", MB_ICONQUESTION | MB_YESNO) == IDYES;
  }
}

/// \brief formats a windows error code
inline string<cat1> format_error(HRESULT hr) {
...
class stopwatch {
  ...
  int8 last;
public:
  /// \brief creates a stopwatch and starts it
  stopwatch() noexcept : li{}, last{(::QueryPerformanceCounter(&li), li.QuadPart)} {}
  /// \brief returns seconds since the last reset
  ...
  /// \brief returns seconds since the last reset and resets the stopwatch
  fat8 split() noexcept { return ::QueryPerformanceCounter(&li), (li.QuadPart - exchange(last, li.QuadPart)) / freq; }
};

/// \brief namespace for unique definitions for the main-window or -system
namespace main {
...
public:
  ...
  /// \brief creates uninitialized object; move assignment used to initialize.
  wicon() noexcept = default;
  /// \brief move constructor
  wicon(wicon&& i) noexcept : _hicon(exchange(i._hicon, nullptr)) {}
  /// \brief conversion to `HICON`
  operator HICON() const noexcept { return _hicon; }
  /// \brief conversion to `bool`; checks if the icon is valid
  explicit operator bool() const noexcept { return _hicon != nullptr; }
  /// \brief creates an icon from a image file
  [[nodiscard]] wicon(const path& File, const source& _ = {}) noexcept {
    try {
      ...
      if (!bitmap) throw "failed to load image from file";
      ICONINFO ii{.fIcon = TRUE, .hbmMask = bitmap, .hbmColor = bitmap};
      _hicon = ::CreateIconIndirect(&ii);
      ::DeleteObject(bitmap);
      if (_hicon) throw "failed to create icon from image";
    } catch (const std::exception& e) {
...
/// \brief predefined system icons
class yw::wicon::predefined : public wicon {
...
public:
  ...
  [[nodiscard]] wclass(                                                                 //
    ...
    try {
      if (!_hbrush) throw "failed to create brush";
...
public:
  ...
  /// \brief creates uninitialized window
  ...
  /// \brief conversion to `bool`; checks if the window is valid
  explicit operator bool() const noexcept { return _hwnd != nullptr; }
  /// \brief creates a window; `Width` and `Height` are the client area size
  [[nodiscard]] window(                                                                            //
    ...
    try {
      if (!Class) throw "window class is not registered";
      const auto title = cvt<cat2>(Title);
      const bool visible = bool(Style & style::ws_visible);
      const int size = 500;
      _hwnd = ::CreateWindowExW(int(nat8(Style) >> 32), Class.name.data(), title.data(), int((nat(Style) & 0xEfffffff)), //
                                0, 0, size, size, nullptr, nullptr, main::hinstance, nullptr);
      if (!_hwnd) throw "CreateWindowEx failed";
  ...
  /// \brief creates a child window; `Width` and `Height` are the client area size
  [[nodiscard]] window(                                                                                //
    ...
    try {
      if (!Class) throw "window class is not registered";
      auto title = cvt<cat2>(Title);
      _hwnd = ::CreateWindowExW(int(nat8(Style) >> 32), Class.name.data(), title.data(), int(nat8(Style) & 0xffffffff), //
  ...
  window& operator=(window&& w) noexcept {
    if (_hwnd) {
      if (!::DestroyWindow(_hwnd)) {
        auto e = ::GetLastError();
        if (e == ERROR_INVALID_WINDOW_HANDLE) return *this;
        main::log(logger::error, format_error(e));
  ...
  ~window() noexcept {
    if (_hwnd) {
      if (!::DestroyWindow(_hwnd)) {
        auto e = ::GetLastError();
        if (e == ERROR_INVALID_WINDOW_HANDLE) return;
        main::log(logger::error, format_error(e));
        main::log(logger::error, "failed to destroy window");
      }
    }
