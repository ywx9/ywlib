...
public:
  ...
  public:
    ...
    virtual float2 get_minimum_draw_size() const noexcept {
      const auto inner = icon_size + float2(icon_offset, 0.0f) + text.size() + padding.xy() + padding.zw();
      const auto outer = float2(ucc.x ? 0.0f : size.x, ucc.y ? 0.0f : size.y);
  ...
  void box(const icon& b) { safe_set(&slot::box, b); }
  ...
  void mark(const icon& b) { safe_set(&slot::mark, b); }
  ...
  void icon_size(const float2& size) { safe_set_size(&slot::icon_size, size); }

  const auto& icon_offset() const { return unsafe_get(&slot::icon_offset); }
  void icon_offset(const float2& offset) { safe_set_size(&slot::icon_offset, offset); }
  ...
  void on_change(const function<void, bool>& f) { safe_set(&slot::on_change, f); }
