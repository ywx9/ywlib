...
public:
  ...
  class slot {
    friend class yw::window;
    friend class yw::window::master;
    ...
    std::expected<void, error_trace> _create_window(const wchar_t* t, window::style s) {
      ...
      hwnd =
        ::CreateWindowExW(WS_EX_ACCEPTFILES, system.name.data(), t, DWORD(s), 0, 0, 0, 0, 0, 0, system.hinstance, 0);
  ...
  static std::expected<window::master, error_trace> open(
...
protected:
  ...
  window(slotlist<slot>::id mid, slotlist<slot>::id sid, bool show) noexcept : _master_id(mid), _slave_id(sid) {
    if (const auto s = _slot()) {
      s->master_id = mid;
      s->slave_id = sid;
      s->timer.start();
      if (show) this->show();
  ...
  slot* _slot() const noexcept {
    if (const auto ms = system.windows.get(_master_id)) return _slave_id.is_zero() ? ms : ms->slaves.get(_slave_id);
    else return nullptr;
  }

  HWND _hwnd() const noexcept {
    if (const auto s = _slot()) return s->hwnd;
    else return nullptr;
  }
...
public:
  ...
  window(window&& w) noexcept
    : _master_id(std::exchange(w._master_id, {})), _slave_id(std::exchange(w._slave_id, {})) {}

  window& operator=(window&& w) noexcept {
    if (this == &w) return *this;
    ::DestroyWindow(_hwnd());
    _master_id = std::exchange(w._master_id, {});
    _slave_id = std::exchange(w._slave_id, {});
    return *this;
  }

  explicit operator bool() const noexcept {
    const auto ms = system.windows.get(_master_id);
    return _slave_id.is_zero() ? bool(ms) : ms->slaves.contains(_slave_id);
  }
  ...
  double time() const noexcept {
    if (const auto s = _slot()) return s->timer.seconds();
    else return 0.0;
  }

  std::expected<drawing, error_trace> begin_draw(const source& src = {}) {
    if (const auto s = _slot()) return s->rendertarget.begin_draw(src);
  ...
  std::expected<drawing, error_trace> begin_draw(color clear_color, const source& src = {}) {
    if (const auto s = _slot()) return s->rendertarget.begin_draw(clear_color, src);
    else return unexpected_error(errors::not_initialized, "window not initialized");
  }

  void screenshot(const std::filesystem::path& Png) const noexcept {
    if (const auto s = _slot()) s->rendertarget.save_as_png(Png);
  }

  void close_confirmation(bool b = true) noexcept {
    if (const auto s = _slot()) s->close_confirmation = b;
  }
};
...
public:
  ...
  std::expected<window, error_trace> open_subwindow(
    ...
    return window(_master_id, sid, Show);
  }
};

inline std::expected<window::master, error_trace> window::open(
  int2 Pos, int2 Size, const null_terminated<wchar_t>& Title, style Style, bool Show) {
  ...
  return window::master(mid, {}, Show);
}

inline std::expected<window::master, error_trace> window::open(
  int2 Size, const null_terminated<wchar_t>& Title, style Style, bool Show) {
  if (auto res = system.initialize(); !res) return unexpected_error(res.error());
  auto ms = std::make_unique<slot>();
  if (auto res = ms->_create_window(Title.data(), Style); !res) return unexpected_error(res.error());
...
protected:
  window::slot* _window() const noexcept {
    if (const auto ms = window::system.windows.get(master_id); slave_id.is_zero()) return ms;
    else return ms ? ms->slaves.get(slave_id) : nullptr;
  }
...
inline LRESULT __stdcall decltype(window::system)::proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_NCDESTROY:
    if (self->slave_id.is_zero()) {
      for (auto& slave : self->slaves) ::DestroyWindow(slave.hwnd);
      system.windows.erase(self->master_id);
      if (system.windows.empty()) ::PostQuitMessage(0);
    } else if (const auto mw = system.windows.get(self->master_id)) mw->slaves.erase(self->slave_id);
    break;
  }
