...
public:
  struct slot : frame::slot {
    ...
    std::expected<void, error> update_text_layout() {
      ...
      hresult_test(
        dwrite::factory()->CreateTextFormat, font.name.value_or(string<wchar_t>()).c_str(), nullptr,
        DWRITE_FONT_WEIGHT(font.weight.value_or(font_weight::normal)),
        DWRITE_FONT_STYLE(font.style.value_or(font_style::normal)),
        DWRITE_FONT_STRETCH(font.stretch.value_or(font_stretch::normal)), font.size.value_or(16.0f), L"", &format);
      ...
      hresult_test(
        dwrite::factory()->CreateTextLayout, text.c_str(), UINT32(text.size()), format, 16384.0f, 16384.0f, &layout);
    ...
    float2 _text_offset() const noexcept {
      ...
      return (size - text_layout_size - padding.xy() - padding.zw()) * cc + padding.xy();
    }

    virtual std::expected<float2, error> get_necessary_size() const override {
      const auto sp = const_cast<slot*>(this);
      if (auto res = sp->ensure_text_layout(); !res) return res.error().relay();
      const auto inner = text_layout_size + padding.xy() + padding.zw();
  ...
  auto& text_color(const color& c) noexcept { ywlib_control_set(this, text_color, c, dirty); }
