...
template<text_layout_like TL> std::expected<void, error_trace> draw_text(float2 pos, TL&& text_layout) {
  ...
  d2d.context()->DrawTextLayout(
    D2D1_POINT_2F{pos.x, pos.y}, tl, brush.d2d_brush(), D2D1_DRAW_TEXT_OPTIONS(text_layout.text_draw_options()));
  return {};
}
...
//     if (hr != E_NOT_SUFFICIENT_BUFFER) return unexpected_error(errors::operation_failed, "HitTestTextRange failed",
