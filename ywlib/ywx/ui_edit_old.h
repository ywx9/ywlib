...
//     void apply_replace(uint32_t First, uint32_t Last, std::wstring_view Inserted, size_t NewCaret) {
//       auto value = text();
//       First = clamp_caret(First);
//       Last = clamp_caret(Last);
//       if (Last < First) std::swap(First, Last);
//       value.replace(First, Last - First, Inserted);
//       text(std::move(value));
//       caret_pos = clamp_caret(NewCaret);
...
//     //   text(std::move(value));
//     //   caret_pos = clamp_caret(new_caret);
//     //   selection_anchor = caret_pos;
//     //   ensure_caret_visible();
//     //   make_messy();
//     //   if (on_change) on_change(text());
...
//       return {yw::min(caret_pos, selection_anchor), yw::max(caret_pos, selection_anchor)};
//     }
...
//         return;
//       }
//       const auto htr = text.hit_test(caret_pos);
//       if (!htr) return;
//       constexpr float caret_w = 1.0f;
//       if (htr->x < scroll_x) scroll_x = htr->x;
//       else if (htr->x + caret_w > scroll_x + iw) scroll_x = htr->x + caret_w - iw;
//       clamp_scroll();
//       request_ime_position_update();
//     }

//     virtual float2 ime_position() const override {
//       if (auto res = text.hit_test(caret_pos)) return pos + padding.xy() + res->xy();
//       return {};
//     }

//     virtual void ime_insert_text(std::wstring_view text) override { insert_text(text); }
...
//       caret_pos = clamp_caret(value);
//       if (!keep_selection) selection_anchor = caret_pos;
//       ensure_caret_visible();
//       make_dirty();
//     }

//     void commit_text(std::wstring value, size_t new_caret) {
//       apply_replace(0, text_length(), value, new_caret);
...
//       if (caret_pos == selection_anchor) return;
...
//       auto current = text();
//       const auto range = selection_range();
//       const auto erase_len = (caret_pos == selection_anchor) ? 0u : (range.y - range.x);
//       const auto base_len = current.size() - erase_len;
//       const auto insert_pos = (caret_pos == selection_anchor) ? caret_pos : range.x;
...
//       if (caret_pos != selection_anchor) erase_range(range.x, range.y, false);
...
//       }
//       if (focused && caret_pos == selection_anchor) {
//         const auto htr = text.hit_test(caret_pos);
//         if (!htr) return;
//         const auto x = text_pos.x + htr->x;
//         const auto h = yw::max(htr->w, yw::max(text.size().y, text.font_size()));
...
//         case key::left:
//           end_typing_group();
//           if (e.ctrl) move_caret(word_left(caret_pos), e.shift);
//           else if (caret_pos > 0) move_caret(caret_pos - 1, e.shift);
//           return true;
//         case key::right:
//           end_typing_group();
//           if (e.ctrl) move_caret(word_right(caret_pos), e.shift);
//           else if (caret_pos < text_length()) move_caret(caret_pos + 1, e.shift);
//           return true;
//         case key::home:
//           end_typing_group();
...
//         case key::backspace:
//           end_typing_group();
//           if (caret_pos != selection_anchor) erase_selection();
//           else if (e.ctrl && caret_pos > 0) erase_at(word_left(caret_pos), caret_pos);
//           else if (caret_pos > 0) erase_at(caret_pos - 1, caret_pos);
//           return true;
//         case key::delete_:
//           end_typing_group();
//           if (caret_pos != selection_anchor) erase_selection();
//           else if (e.ctrl && caret_pos < text_length()) erase_at(caret_pos, word_right(caret_pos));
//           else if (caret_pos < text_length()) erase_at(caret_pos, caret_pos + 1);
//           return true;
//         default: break;
//         }
//         if (e.first && on_keydown) on_keydown(e);
...
//       csp->text(std::move(s));
//       csp->caret_pos = csp->clamp_caret(csp->caret_pos);
...
//     if (auto csp = system::slot_address<slot>(_id)) csp->set_caret(value.x);
//   }
...
//         csp->commit_text(std::move(clipped), yw::min(size_t(csp->caret_pos), limit));
//       } else csp->make_dirty();
//     }
//   }
