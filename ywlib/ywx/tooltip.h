...
public:
  ...
  std::expected<void, error_trace> show(int2 Pos, uint2 Size, std::wstring Text) {
    _visible = true;
    if (auto res = initialize(); !res) return unexpected_error(res.error());
    text.handle().string(std::move(Text)).padding({8, 4, 8, 4});
    _window_size = text.layout_size;
    if (_swapchain) {
      _rendertarget = {};
    ...
    if (auto d = _rendertarget.begin_draw(colors::whitesmoke)) {
      ...
      if (auto res = text.draw({}, _window_size); !res) return unexpected_error(res.error());
