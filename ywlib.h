...
template<typename F, typename R, typename... As> concept invocable_r = invocable<F, As...> && convertible_to<invoke_result_t<F, As...>, R>;
template<typename F, typename R, typename... As> concept nt_invocable_r = nt_invocable<F, As...> && nt_convertible_to<invoke_result_t<F, As...>, R>;
...
struct t_vapply {
  template<natt I, typename Fn, typename... Ts> static constexpr auto call_i(constant<I>, Fn&& f, Ts&&... ts)
    noexcept((nt_gettable<Ts, I> && ...) && nt_invocable<Fn&, element_t<Ts, I>...>) -> decltype(auto) { return invoke(f, get<I>(fwd<Ts>(ts)...)); }
  template<natt... Is, typename Fn, tuple... Ts> static constexpr auto call(sequence<Is...>, Fn&& f, Ts&&... ts)
    ywlib_wrapper(list<>::asref(call_i(constant<Is>{}, f, fwd<Ts>(ts)...)...));
  ...
  template<sequence_of<natt> S, typename Fn, tuple... Ts> constexpr auto operator()(S, Fn&& f, Ts&&... ts) const
    ywlib_wrapper(call(to_sequence<S, natt>{}, fwd<Fn>(f), fwd<Ts>(ts)...));
  template<typename Fn, tuple T, tuple... Ts> constexpr auto operator()(Fn&& f, T&& t, Ts&&... ts) const
  requires requires { call(make_indices_for<T>{}, fwd<Fn>(f), fwd<T>(t), fwd<Ts>(ts)...); }
    { return call(make_indices_for<T>{}, fwd<Fn>(f), fwd<T>(t), fwd<Ts>(ts)...); }
    // ywlib_wrapper(call(make_indices_for<T>{}, fwd<Fn>(f), fwd<T>(t), fwd<Ts>(ts)...));
// private:
};
...
template<typename Fn, typename... Ts> requires vapplyable<Fn, Ts...> using vapply_result_t = decltype(vapply(declval<Fn>(), declval<Ts>()...));
...
template<invocable... Fs> struct caster : public Fs... {
  template<typename T> static constexpr natt i = []<typename... Ts>(typepack<Ts...>) { return inspects<same_as<Ts, T>...>; }(typepack<invoke_result_t<Fs>...>{});
...
public:
  template<typename T> requires(j<T> < sizeof...(Fs)) constexpr operator T() const
    noexcept(nt_convertible_to<invoke_result_t<type_switch<j<T>, Fs...>>, T>) { return type_switch<j<T>, Fs...>::operator()(); }
};
...
// template<yw::iterator It, yw::iter_unary_invocable<It> Pj>
// struct incrementable_traits<yw::projector<It, yw::sequence<>, Pj>> { using difference_type = yw::iter_difference_t<It>; };
} // clang-format on
