...
protected:
  ...
  std::expected<void, error_trace> _calculate_padding() {
    ...
    const auto pad = (window_rect.right - window_rect.left - client_rect.right) / 2; // left, right and bottom are same
    const auto pad_top = window_rect.bottom - window_rect.top - client_rect.bottom - pad;
    _pad = int4{pad, pad_top, 2 * pad, pad + pad_top};
    return {};
  }
  ...
  std::expected<void, error_trace> _move_to_center() {
    if (auto desktop = ::GetDesktopWindow(); !desktop)
      ...
      return unexpected_error(errors::operation_failed, "GetClientRect failed", int32_t(::GetLastError()));
               ...
               _hwnd, nullptr, (r.right - wh.x) / 2, (r.bottom - wh.y) / 2, wh.x, wh.y, SWP_NOZORDER | SWP_NOACTIVATE))
...
public:
  ...
  std::expected<void, error_trace> size(uint2 Size) {
    ...
    if (::SetWindowPos(_hwnd, nullptr, 0, 0, w, h, SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOMOVE)) return {};
...
public:
  ...
  std::expected<void, error_trace> close() {
    if (auto w = _get(); !w) return unexpected_error(errors::invalid_operation, "window not found");
    else window_class.active_windows.erase(_id);
    return {};
  }
...
inline std::expected<subwindow_handle, error_trace> subwindow::create(const window_handle& parent, int2 pos, uint2 size,
  ...
  for (auto id : window_class.active_windows) {
    if (auto wnd = window_class.active_windows.get(id); wnd) {
      ...
      for (auto sub_id : wnd->subwindows)
        if (auto subwnd = wnd->subwindows.get(sub_id); subwnd && subwnd->_swapchain) subwnd->_swapchain->Present(1, 0);
    }
  }
