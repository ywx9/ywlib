...
public:
  ...
  template<typename Self> requires invocable_r<
    decltype(&Self::_make_mesh), std::expected<void, error>, Self&, const typename Self::remesh_option&>
  std::expected<void, error> update_gpu(this Self& self, const typename Self::remesh_option& ro) noexcept {
