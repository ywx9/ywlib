...
/// MARK: geom::geometry2_base

template<typename Geometry, backend Backend> class geometry2_base : public geometry_base<Geometry, Backend> {};

/// MARK: geom::segment

/// represents a 2D line segment as a transformed unit segment from (0, 0) to (1, 0).
template<backend Backend> class segment : public geometry2_base<segment<Backend>, Backend> {
public:
...
template<backend Backend> class ray : public geometry2_base<ray<Backend>, Backend> {
public:
...
template<backend Backend> class line : public geometry2_base<line<Backend>, Backend> {
public:
  struct remeshing_option_t {
...
template<backend Backend> class plane : public geometry2_base<plane<Backend>, Backend> {
public:
  struct remeshing_option_t {
...
template<backend Backend> class square : public geometry2_base<square<Backend>, Backend> {
public:
  struct remeshing_option_t {}; // no options for remeshing
...
template<backend Backend> class circle : public geometry2_base<circle<Backend>, Backend> {
public:
  struct remeshing_option_t {
...
template<backend Backend> class polygon : public geometry2_base<polygon<Backend>, Backend> {
public:
  struct remeshing_option_t {}; // no options for remeshing
  static constexpr bool has_bounded_surface = true;
...
public:
  ...
  std::expected<void, error> set(size_t index, const double2& point) noexcept {
    if (index >= _points.size()) return std::unexpected(error(errors::invalid_argument, "polygon point index out of bounds"));
    _points[index] = {point.x, point.y, 0, 1};
    this->_messy = true;
    return {};
  }
  std::expected<void, error> set(size_t index, const double3& point) noexcept {
    return set(index, point.xy());
  }
  std::expected<void, error> set(size_t index, const double4& point) noexcept {
    return set(index, point.xy());
  }
...
template<backend Backend> class arc : public geometry2_base<arc<Backend>, Backend> {
public:
  struct remeshing_option_t {
    uint1 subdivisions = 32;
  };
...
template<backend Backend> class quadratic_bezier : public geometry2_base<quadratic_bezier<Backend>, Backend> {
public:
  struct remeshing_option_t {
    uint1 subdivisions = 32;
  };
...
public:
  ...
  constexpr quadratic_bezier(const double2& p0, const double2& p1, const double2& p2) noexcept {
    points(p0, p1, p2);
  }
  constexpr quadratic_bezier(const double3& p0, const double3& p1, const double3& p2) noexcept {
    points(p0, p1, p2);
  }
  constexpr quadratic_bezier(const double4& p0, const double4& p1, const double4& p2) noexcept {
    points(p0, p1, p2);
  }
...
template<backend Backend> class cubic_bezier : public geometry2_base<cubic_bezier<Backend>, Backend> {
public:
  struct remeshing_option_t {
    uint1 subdivisions = 32;
  };
...
protected:
  ...
  double4 _p0 = {-1, 0, 0, 1};
  double4 _p1 = {-0.5, 1, 0, 1};
  double4 _p2 = {0.5, -1, 0, 1};
  double4 _p3 = {1, 0, 0, 1};
  remeshing_option_t _remeshing_option{};
  std::expected<void, error> _make_mesh(const remeshing_option_t& ro) noexcept;
  std::expected<void, error> _make_mesh_default() noexcept { return _make_mesh(_remeshing_option); }
...
public:
  ...
  constexpr void points(const double2& p0, const double2& p1, const double2& p2, const double2& p3) noexcept {
    points(
      double4(p0.x, p0.y, 0, 1), double4(p1.x, p1.y, 0, 1), double4(p2.x, p2.y, 0, 1),
      double4(p3.x, p3.y, 0, 1));
  }
