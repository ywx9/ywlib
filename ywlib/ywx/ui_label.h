...
public:
  struct slot : label::slot {
    ...
    uint32_t _char_index_from_point(float2 pt) const noexcept {
      const auto local = pt - (pos + _text_offset());
    ...
    virtual std::optional<float3> caret_pos() const override {
      const auto origin = pos + _text_offset();
      ...
      if (auto res = text.hittest(uint1{index}))
        return float3(origin.x + res->pos.x + res->size.x, origin.y + res->pos.y, res->size.y);
      return std::nullopt;
    }
    ...
    std::expected<void, error> _draw_selection() {
      ...
      if (auto res = text.hittest_range(_selected_range(), pos + _text_offset()); res) {
    ...
    std::expected<void, error> _draw_caret() {
      if (!caret_visible || caret_color.a <= 0.0f || caret_thickness <= 0.0f) return {};
      const auto origin = pos + _text_offset();
