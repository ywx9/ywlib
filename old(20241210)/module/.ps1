...
# if `--ixx` argument is passed, compiles `ywlib` module
if ($Args -contains "--ixx") {
  Write-Host "[TASK] Compiling ywlib module"
  Remove-Item -Path ywlib.ifc -ErrorAction SilentlyContinue
  Invoke-Expression "&`"$($e.cl)`" /std:c++latest /EHsc /nologo /W4 /O2 /Qpar /utf-8 /DYWLIB=1 /c module/.ixx /Foywlib.obj /ifcOutputywlib.ifc /I`"$($e.msvc_inc)`" /I`"$($e.ucrt_inc)`" /I`"$($e.um_inc)`" /I`"$($e.shared_inc)`" /I`"$($e.winrt_inc)`" /I`"$($e.cppwinrt_inc)`""
  if (-not ((Test-Path ywlib.ifc) -and (Test-Path ywlib.obj))) {
    Write-Host "[TASK] Failed to compile ywlib module"
    exit 1
  } else {
    Write-Host "[TASK] ywlib module compiled"
  }
}
