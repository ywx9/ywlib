...
int main() {
  if (!main::system::hwnd) return ok(L"ウィンドウが作成されていません", L"エラー");
  if (!main::system::d3d_device) return ok(L"Direct3Dデバイスが作成されていません", L"エラー");
  if (!main::system::d3d_context) return ok(L"Direct3Dコンテキストが作成されていません", L"エラー");
  if (!main::system::dxgi_factory) return ok(L"DXGIファクトリが作成されていません", L"エラー");
  if (!main::system::swapchain) return ok(L"DXGIスワップチェーンが作成されていません", L"エラー");
  if (!main::system::d2d_factory) return ok(L"Direct2Dファクトリが作成されていません", L"エラー");
  if (!main::system::d2d_device) return ok(L"Direct2Dデバイスが作成されていません", L"エラー");
  if (!main::system::d2d_context) return ok(L"Direct2Dコンテキストが作成されていません", L"エラー");
  if (!main::system::rendertarget) return ok(L"レンダーターゲットが作成されていません", L"エラー");
  if (!main::system::dw_factory) return ok(L"DirectWriteファクトリが作成されていません", L"エラー");
  if (!main::system::wic_factory) return ok(L"WICファクトリが作成されていません", L"エラー");

  int i{};

  while (main::update) {
    // std::cout << int(main::system::keyboard_state[2]['A']) << std::endl;
    // if (key::oem10) ok("oem10");
    // if (key::a) ok("a");
    // if (key::space) ok("space");
    if (key::a.free) std::cout << "a is free\n";
    else if (key::a.up) std::cout << "a is up\n";
    else if (key::a.down) std::cout << "a is down\n";
    else if (key::a.hold) std::cout << "a is hold\n";
    if (key::escape) break;
  }
  main::terminate();
}
...
//       uint j = 3 * i;
//       for (uint k{}; k < 15; ++k) {
//         if (abs(cb[k].w) == 1) ub0[i].d[k] = f0(sb0[j].v.yzx - cb[k].yzx, sb0[j + 1].v.yzx - cb[k].yzx, sb0[j + 2].v.yzx - cb[k].yzx);
//         else if (abs(cb[k].w) == 2) ub0[i].d[k] = f0(sb0[j].v.zxy - cb[k].zxy, sb0[j + 1].v.zxy - cb[k].zxy, sb0[j + 2].v.zxy - cb[k].zxy);
...
//   // 成績
//   draw_text({950, 680, 1250, 705}, font<20, L"Yu Gothic UI", -1>{}, std::format(L"面上最小取代： {:.2f} mm", apply(min, projector(MaxMin.mins, &vector::w, make_sequence<25>{}))));
